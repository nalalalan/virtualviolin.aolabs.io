...
assert.equal(stringForAngle(-33, 'A'), 'E')
assert.equal(getStringForBowVector(-86.6, 50, 'A'), 'G')
assert.equal(getStringForBowVector(-98.5, 17.4, 'A'), 'D')
assert.equal(stringForAngle(28, 'D'), 'D', 'D keeps more room before crossing to G')
assert.equal(stringForAngle(32, 'D'), 'D', 'D holds through the steeper G/D threshold')
assert.equal(stringForAngle(33, 'D'), 'G', 'D switches to G only after the steeper deliberate angle crossing')
...
assert.equal(stringForAngle(-32, 'A'), 'A', 'A holds through the steeper A/E threshold')
assert.equal(stringForAngle(-33, 'A'), 'E', 'A switches to E only after the steeper deliberate angle crossing')
assert.equal(stringForAngle(20, 'G'), 'G', 'G keeps the string through small jitter toward D')
assert.equal(stringForAngle(17, 'G'), 'D', 'G switches to D after crossing the lower guard band')
assert.equal(stringForAngle(-20, 'E'), 'E', 'E keeps the string through small jitter toward A')
assert.equal(stringForAngle(-17, 'E'), 'A', 'E switches to A after crossing the upper guard band')
const dSelection = createBowStringSelectionState('D')
assert.equal(updateSelectionForAngle(dSelection, 33), 'D', 'one noisy frame above G/D does not switch')
assert.equal(updateSelectionForAngle(dSelection, 28), 'D', 'returning to the D-side band clears the pending switch')
assert.equal(updateSelectionForAngle(dSelection, 33), 'D', 'first deliberate G frame still latches D')
assert.equal(updateSelectionForAngle(dSelection, 33), 'G', 'sustained crossing switches to G')
...
assert.equal(updateSelectionForAngle(aSelection, 33), 'A', 'single far-angle frame does not jump across adjacent strings')
assert.equal(updateSelectionForAngle(aSelection, 33), 'D', 'far-angle crossing steps from A to D first')
assert.equal(updateSelectionForAngle(aSelection, 33), 'D', 'first continued frame toward G latches D')
assert.equal(updateSelectionForAngle(aSelection, 33), 'G', 'continued far-angle crossing reaches G deliberately')
