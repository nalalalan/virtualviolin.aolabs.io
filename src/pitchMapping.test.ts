...
assert.equal(stringForAngle(-44, 'A'), 'A')
assert.equal(stringForAngle(-45, 'A'), 'E')
assert.equal(getStringForBowVector(-86.6, 50, 'A'), 'G')
assert.equal(getStringForBowVector(-98.5, 17.4, 'A'), 'D')
assert.equal(stringForAngle(28, 'D'), 'D', 'D keeps more room before crossing to G')
assert.equal(stringForAngle(44, 'D'), 'D', 'D holds through the protected G/D entry range')
assert.equal(stringForAngle(45, 'D'), 'G', 'D switches to G only after a deliberate outer-string crossing')
...
assert.equal(stringForAngle(-44, 'A'), 'A', 'A holds through the protected A/E entry range')
assert.equal(stringForAngle(-45, 'A'), 'E', 'A switches to E only after a deliberate outer-string crossing')
...
assert.equal(updateSelectionForAngle(dSelection, 45), 'D', 'one noisy frame above G/D does not switch')
assert.equal(updateSelectionForAngle(dSelection, 28), 'D', 'returning to the D-side band clears the pending switch')
assert.equal(updateSelectionForAngle(dSelection, 45), 'D', 'first deliberate G frame still latches D')
assert.equal(updateSelectionForAngle(dSelection, 45), 'G', 'sustained crossing switches to G')
...
assert.equal(updateSelectionForAngle(aSelection, 45), 'A', 'single far-angle frame does not jump across adjacent strings')
assert.equal(updateSelectionForAngle(aSelection, 45), 'D', 'far-angle crossing steps from A to D first')
assert.equal(updateSelectionForAngle(aSelection, 45), 'D', 'first continued frame toward G latches D')
assert.equal(updateSelectionForAngle(aSelection, 45), 'G', 'continued far-angle crossing reaches G deliberately')
