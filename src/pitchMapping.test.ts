...
assert.equal(updateSelectionForAngle(dSelection, 28), 'D', 'one noisy frame above G/D does not switch')
assert.equal(updateSelectionForAngle(dSelection, 18), 'D', 'returning to the guard band clears the pending switch')
assert.equal(updateSelectionForAngle(dSelection, 28), 'D', 'first deliberate G frame still latches D')
assert.equal(updateSelectionForAngle(dSelection, 28), 'G', 'sustained crossing switches to G')
...
assert.equal(updateSelectionForAngle(aSelection, 30), 'A', 'single far-angle frame does not jump across adjacent strings')
assert.equal(updateSelectionForAngle(aSelection, 30), 'D', 'far-angle crossing steps from A to D first')
assert.equal(updateSelectionForAngle(aSelection, 30), 'D', 'first continued frame toward G latches D')
assert.equal(updateSelectionForAngle(aSelection, 30), 'G', 'continued far-angle crossing reaches G deliberately')
